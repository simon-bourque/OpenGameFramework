...
Texture* TextureManager::createTexture2DArray(const string& name, const string& infoName, uint32* frames, uint32 framesSize, Texture::Filter filtering, Texture::Wrap textureWrapS, Texture::Wrap textureWrapT) {

	//uint32 layers = 0;
	uint32 numImages = 0;
	RawImage* imgs = loadImages(name, infoName, numImages);
	uint32 width = imgs[frames[0]].getWidth();
	uint32 height = imgs[frames[0]].getHeight();
	uint32 subImgSize = width * height * imgs[frames[0]].getChannels();

	uint8* data = new uint8[subImgSize * framesSize];

	// TODO Ensure all images are the same size
	for (uint32 i = 0; i < framesSize; i++) {
		uint8* subImgData = imgs[frames[i]].getData();
