...
TileScene::TileScene(Tile* tiles, int32 numberOfTiles, Texture* tileSheet, const Rectangle& bounds) : Scene(bounds), m_tileSheet(tileSheet), m_numberOfTiles(numberOfTiles) {
	...
	m_tileVAO = new VertexArrayObject();
...
void TileScene::render(const RenderSystem& rs) {
