...
void ShapeRenderer::drawLine(float32 xi, float32 yi, float32 xf, float32 yf, const Color& color) {
	...
	glUniformMatrix3fv(m_shapeShaderProgram->getUniform("mvpMatrix").getLocation(), 1, true, getGraphics2DInstance()->getCamera().getViewProjectionMatrix().values);
...
void ShapeRenderer::drawRectangle(float32 x, float32 y, float32 width, float32 height, const Color& color, bool filled) {
	...
	Matrix3f finalMatrix = getGraphics2DInstance()->getCamera().getViewProjectionMatrix() * Matrix3f::translation(x, y) * Matrix3f::scale(width, height);
